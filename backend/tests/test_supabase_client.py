...
def test_environment_variables_missing():
    ...
    with patch.dict(os.environ, {}, clear=True):
        with pytest.raises(
            ValueError,
            match="SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY environment variables are required",
        ):
            supabase_client.get_supabase_client()

        with pytest.raises(
            ValueError,
            match="SUPABASE_URL and SUPABASE_ANON_KEY environment variables are required",
        ):
...
def test_service_role_client_purpose(mock_create_client):
    ...
    assert (
        supabase_client.get_supabase_client()
        is not supabase_client.get_supabase_auth_client()
    )
...
def test_anon_client_purpose(mock_create_client):
    ...
    assert (
        supabase_client.get_supabase_auth_client()
        is not supabase_client.get_supabase_client()
    )
