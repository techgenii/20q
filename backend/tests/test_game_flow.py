...
def test_make_guess_correct(monkeypatch):
    ...
    monkeypatch.setattr(
        game_logic, "get_game", lambda game_id: {"secret_word": "elephant"}
    )
    # Patch update_game_winner to do nothing
    monkeypatch.setattr(
        game_logic, "update_game_winner", lambda game_id, player_id: None
    )
...
def test_get_game_audio_settings(monkeypatch):
    monkeypatch.setattr(
        game_logic,
        "get_game",
        lambda game_id: {"enable_tts": True, "voice_id": "voice-123"},
    )
...
def test_update_game_tts_settings(monkeypatch):
    mock_response = MagicMock()
    mock_response.data = [
        {"id": "game-uuid", "enable_tts": True, "voice_id": "voice-123"}
    ]
    mock_supabase = MagicMock()
    mock_supabase.table.return_value.update.return_value.eq.return_value.execute.return_value = (
        mock_response
    ...
    monkeypatch.setattr(game_logic, "get_supabase_client", lambda: mock_supabase)
    result = game_logic.update_game_tts_settings(
        "game-uuid", enable_tts=True, voice_id="voice-123"
    )
    assert result["enable_tts"] is True
    assert result["voice_id"] == "voice-123"
...
def test_get_or_create_player_stats_difficulty(monkeypatch):
    mock_response = MagicMock()
    mock_response.data = [
        {"player_id": "player-uuid", "difficulty": 1, "games_played": 1}
    ]
    mock_supabase = MagicMock()
...
def test_upsert_player_stats_difficulty(monkeypatch):
    ...
    monkeypatch.setattr(game_logic, "get_supabase_client", lambda: mock_supabase)
    game_logic.upsert_player_stats_difficulty(
        "player-uuid", 1, {"player_id": "player-uuid"}
    )
...
def test_ask_question_with_tts(monkeypatch):
    monkeypatch.setattr(
        game_logic,
        "get_game",
        lambda game_id: {"secret_word": "test", "enable_tts": True, "voice_id": "v1"},
    )
    monkeypatch.setattr(
        game_logic,
        "ask_openai_question",
        lambda *a, **kw: {"answer": "Yes", "audio": "base64"},
    )
...
def test_make_guess_with_tts(monkeypatch):
    monkeypatch.setattr(
        game_logic, "get_game", lambda game_id: {"enable_tts": True, "voice_id": "v1"}
    )
    monkeypatch.setattr(
        game_logic,
        "make_guess",
        lambda *a, **kw: {"correct": True, "message": "Congrats!"},
    )
...
def test_start_game_with_game_type_max_players_guessed_word(monkeypatch):
    ...
    result = game_logic.start_game(
        "host", 1, game_type="solo", max_players=2, guessed_word="cat"
    )
