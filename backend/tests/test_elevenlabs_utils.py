...
def patch_environment_variables(monkeypatch):
    ...
    monkeypatch.setattr(
        elevenlabs_utils, "ELEVENLABS_BASE_URL", "https://api.elevenlabs.io/v1"
    )
...
def test_generate_speech_with_custom_voice():
    ...
    with patch("requests.post") as mock_post:
        ...
        result = elevenlabs_utils.generate_speech(
...
def test_generate_game_message_audio_with_difficulty():
    ...
    with patch("requests.post") as mock_post:
        ...
        result = elevenlabs_utils.generate_game_message_audio(
            "welcome_with_difficulty", difficulty=3
        )
